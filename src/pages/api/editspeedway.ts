...
export default authenticated (async (request: NextApiRequest, response: NextApiResponse) => {
    ...
    if(request.method === 'PUT'){
        ...
        const {data:speedway, vehicles_limit, description, speedwayId} = request.body
        ...
        try{
            ...
            const updatedData = await fauna.query(
                q.If(
                    q.Exists(
                      q.Match(q.Index("speedway_by_name"), speedway)
                    ),
                  q.Abort(`This company name is already in use`),
